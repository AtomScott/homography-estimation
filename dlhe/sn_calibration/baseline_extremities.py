...
class SegmentationNetwork:
    def __init__(
        self, model_file, mean_file, std_file, num_classes=29, width=640, height=360
    ):
        file_path = Path(model_file).resolve()
        model = nn.DataParallel(
            deeplabv3_resnet50(pretrained=False, num_classes=num_classes)
        )
        self.init_weight(
            model, nn.init.kaiming_normal_, nn.BatchNorm2d, 1e-3, 0.1, mode="fan_in"
        )
    ...
    def init_weight(
        self, feature, conv_init, norm_layer, bn_eps, bn_momentum, **kwargs
    ):
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        type=bool,
        default=False,
        help="Save masks in prediction directory",
    )
    parser.add_argument(
        "--resolution_width",
        required=False,
    ...
    with tqdm(enumerate(frames), total=len(frames), ncols=160) as t:
        for i, frame in t:
            ...
            prediction_file = os.path.join(
                output_prediction_folder, f"extremities_{frame_index}.json"
            )
