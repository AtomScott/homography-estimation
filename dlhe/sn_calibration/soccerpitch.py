...
class SoccerPitch:
    ...
    def __init__(self, pitch_length=105.0, pitch_width=68.0):
        ...
        self.halfway_line_and_center_circle_top_mark = np.array(
            [0, -SoccerPitch.CENTER_CIRCLE_RADIUS, 0], dtype="float"
        )
        ...
        self.left_goal_bottom_left_post = np.array(
            [-pitch_length / 2.0, SoccerPitch.GOAL_LENGTH / 2.0, 0.0], dtype="float"
        )
        self.left_goal_top_left_post = np.array(
            [
                -pitch_length / 2.0,
                SoccerPitch.GOAL_LENGTH / 2.0,
                -SoccerPitch.GOAL_HEIGHT,
            ],
            dtype="float",
        )
        ...
        self.left_penalty_mark = np.array(
            [-pitch_length / 2.0 + SoccerPitch.GOAL_LINE_TO_PENALTY_MARK, 0, 0],
            dtype="float",
        )
        ...
        self.left_penalty_area_top_right_corner = np.array(
            [
                ...
                -SoccerPitch.PENALTY_AREA_WIDTH / 2.0,
                0,
            ],
            dtype="float",
        )
        ...
        self.right_penalty_area_bottom_right_corner = np.array(
            [pitch_length / 2.0, SoccerPitch.PENALTY_AREA_WIDTH / 2.0, 0], dtype="float"
        )
        ...
        self.left_goal_area_top_right_corner = np.array(
            [
                -pitch_length / 2.0 + SoccerPitch.GOAL_AREA_LENGTH,
                -SoccerPitch.GOAL_AREA_WIDTH / 2.0,
                0,
            ],
            dtype="float",
        )
        self.left_goal_area_top_left_corner = np.array(
            [-pitch_length / 2.0, -SoccerPitch.GOAL_AREA_WIDTH / 2.0, 0], dtype="float"
        )
        ...
        y = -np.sqrt(
            SoccerPitch.CENTER_CIRCLE_RADIUS * SoccerPitch.CENTER_CIRCLE_RADIUS
            - dx * dx
        )
        ...
        self.line_extremities_keys["Side line top"] = (
            "TL_PITCH_CORNER",
            "TR_PITCH_CORNER",
        )
        self.line_extremities_keys["Side line bottom"] = (
            "BL_PITCH_CORNER",
            "BR_PITCH_CORNER",
        )
        self.line_extremities_keys["Side line left"] = (
            "TL_PITCH_CORNER",
            "BL_PITCH_CORNER",
        )
        self.line_extremities_keys["Side line right"] = (
            "TR_PITCH_CORNER",
            "BR_PITCH_CORNER",
        )
        ...
        self.line_extremities_keys["Goal left crossbar"] = (
            "L_GOAL_TR_POST",
            "L_GOAL_TL_POST",
        )
        self.line_extremities_keys["Goal left post left "] = (
            "L_GOAL_TL_POST",
            "L_GOAL_BL_POST",
        )
        self.line_extremities_keys["Goal left post right"] = (
            "L_GOAL_TR_POST",
            "L_GOAL_BR_POST",
        )

        self.line_extremities_keys["Goal right crossbar"] = (
            "R_GOAL_TL_POST",
            "R_GOAL_TR_POST",
        )
        self.line_extremities_keys["Goal right post left"] = (
            "R_GOAL_TL_POST",
            "R_GOAL_BL_POST",
        )
        self.line_extremities_keys["Goal right post right"] = (
            "R_GOAL_TR_POST",
            "R_GOAL_BR_POST",
        )
        ...
        for key, line in self.line_extremities.items():
            ...
            if "Circle" in key:
                if key == "Circle right":
                    ...
                    toAngle = (
                        np.arctan2(top[1] - center[1], top[0] - center[0]) + 2 * np.pi
                    )
                    fromAngle = (
                        np.arctan2(bottom[1] - center[1], bottom[0] - center[0])
                        + 2 * np.pi
                    )
