...
from .evaluate_extremities import distance, mirror_labels, scale_points
...
def distance_to_polyline(point, polyline):
    ...
    if 0 < len(polyline) < 2:
        ...
        for i in range(len(polyline) - 1):
            origin_segment = np.array([polyline[i]["x"], polyline[i]["y"], 1])
            end_segment = np.array([polyline[i + 1]["x"], polyline[i + 1]["y"], 1])
