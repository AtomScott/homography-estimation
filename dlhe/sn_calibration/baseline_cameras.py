...
def draw_pitch_homography(image, homography):
    ...
    for line in polylines.values():
        ...
        for point in line:
            if point[2] == 0.0:
                hp = np.array((point[0], point[1], 1.0))
                ...
                if 0 < projected[0] < 960 and 0 < projected[1] < 540:
                    cv.circle(
                        image, (int(projected[0]), int(projected[1])), 1, (255, 0, 0), 1
                    )
    ...
    with tqdm(enumerate(frames), total=len(frames), ncols=160) as t:
        for i, frame in t:
            ...
            for k, v in predictions.items():
                ...
                p1 = np.array(
                    [
                        v[0]["x"] * args.resolution_width,
                        v[0]["y"] * args.resolution_height,
                        1.0,
                    ]
                )
                p2 = np.array(
                    [
                        v[1]["x"] * args.resolution_width,
                        v[1]["y"] * args.resolution_height,
                        1.0,
                    ]
                )
                ...
                if success:
                    ...
                    if success:
                        ...
                        for k, potential_matches in potential_3d_2d_matches.items():
                            ...
                            if (
                                0 < projected[0] < args.resolution_width
                                and 0 < projected[1] < args.resolution_height
                            ):
